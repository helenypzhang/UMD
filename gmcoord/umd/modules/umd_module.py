...
class UMDTransformerSS(pl.LightningModule):
    def __init__(self, config):
        ...
        # init teacher model weights
        # self.teacher_model_weights = [w.detach().clone().to(self.device) for w in self.parameters()]

        # init gradient modulation coefficient
        self.coeff_t = None
        self.coeff_i = None
        self.warmup_steps_num = None
    ...
    def forward(self, batch, test=False):
        ...
        # print('current_tasks:', self.current_tasks)
        ...
        if "pretrain" in self.hparams.config["exp_name"]:
            ##(new)ema for unmasked infer
            # with torch.no_grad():
            #     ema_infer_unmask = self.ema.module.infer(batch, mask_text=False, mask_image=False)
        ...
        # if "vqa" in self.current_tasks or "cls" in self.current_tasks or "irtr" in self.current_tasks:
        if "finetune" in self.hparams.config["exp_name"]:
            # step1: for train
            # with torch.no_grad():
            #     infer_unmask = self.infer(batch, mask_text=False, mask_image=False)
            infer_unmask = self.infer(batch, mask_text=False, mask_image=False)
            ...
            # infer_masks = self.infer(batch, mask_text=True, mask_image=True)
            # with torch.no_grad():
            #     infer_masks_preteacher = self.preteacher.module.infer(batch, mask_text=True, mask_image=True)
        ...
        if "bb" in self.current_tasks:
            # ret.update(objectives.compute_mlmfr(self, batch, infer_unmask, infer_masks))
            ret.update(objectives.compute_bb(self, infer_masks))
        ...
        if "vqa" in self.current_tasks:
            ret.update(objectives.compute_vqa(self, batch, infer_unmask, test=test))

        # Fine-Tuning: Image-Text Classification
        if "cls" in self.current_tasks:
            ret.update(objectives.compute_cls(self, batch, infer_unmask, test=test))
