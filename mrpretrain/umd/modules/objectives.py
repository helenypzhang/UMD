...
def compute_feal(pl_module, infer_masks, infer_unmask):
    ...
    feal_logits = pl_module.feal_proj(feal_logits)
    feal_labels = pl_module.feal_proj(feal_labels)

    feal_loss = (feal_logits - feal_labels) ** 2
    feal_loss = torch.mean(feal_loss)  # [N, L], mean loss per patch

    feal_loss = feal_loss * 0.5
...
def compute_feai(pl_module, infer_masks, infer_unmask):
    ...
    ## for infer_unmask, for allpreema_continue 1:1
    if pl_module.hparams.config["mim_layer"] == -1:
        unmask_multi_modal_image_feats = infer_unmask["multi_modal_image_feats"]
    else:
        layer_idx = pl_module.hparams.config["mim_layer"]
        unmask_multi_modal_image_feats = infer_unmask[f"multi_modal_image_feats_{layer_idx}"]

    # for allpreema_layer 3:1
    # unmask_multi_modal_image_feats = infer_unmask["multi_modal_image_feats"]
    ...
    # # 以下为改进部分：使用L1 loss替代MSE
    # feai_loss = torch.abs(feai_logits - feai_labels)
...
def compute_itc(pl_module, infer_masks):
    ...
    text_feats = infer_masks["multi_modal_text_cls_feats"]
    image_feats = infer_masks["multi_modal_image_cls_feats"]
...
def compute_bb(pl_module, infer_masks, infer_unmask):
    ...
    text_unmask = infer_unmask["multi_modal_text_cls_feats"]
    image_unmask = infer_unmask["multi_modal_image_cls_feats"]
    text_mask = infer_masks["multi_modal_text_cls_feats"]
    image_mask = infer_masks["multi_modal_image_cls_feats"]

    text_unmask = torch.cat(GatherLayer.apply(text_unmask), dim=0)
    image_unmask = torch.cat(GatherLayer.apply(image_unmask), dim=0)
    text_mask = torch.cat(GatherLayer.apply(text_mask), dim=0)
    image_mask = torch.cat(GatherLayer.apply(image_mask), dim=0)
    ...
    batch_loss = (batch_loss_unmask + batch_loss_mask) * 1000
