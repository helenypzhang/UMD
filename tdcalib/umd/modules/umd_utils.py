...
from ..gadgets.my_metrics import Accuracy, Scalar, VQARADScore, ROCScore, F1Score, ALLScore, EVALLScore
        ...
        for k, v in pl_module.hparams.config["loss_names"].items():
            ...
            if k == "vqa":
                ...
                if split == "train":
                    ...
                    setattr(pl_module, f"train_{k}_evallscore", EVALLScore())
                    # setattr(pl_module, f"train_{k}_roc", ROCScore())
                    # setattr(pl_module, f"train_{k}_f1", F1Score())
                    # setattr(pl_module, f"train_{k}_evall", ALLScore())
                    setattr(pl_module, f"train_{k}_loss", Scalar())
                else:
                    setattr(pl_module, f"val_{k}_accuracy", Accuracy())
                    setattr(pl_module, f"val_{k}_evallscore", EVALLScore())
                    # setattr(pl_module, f"val_{k}_roc", ROCScore())
                    # setattr(pl_module, f"val_{k}_f1", F1Score())
                    # setattr(pl_module, f"val_{k}_evall", ALLScore())
                    setattr(pl_module, f"val_{k}_loss", Scalar())

                    setattr(pl_module, f"test_{k}_accuracy", Accuracy())
                    setattr(pl_module, f"test_{k}_evallscore", EVALLScore())
                    # setattr(pl_module, f"test_{k}_roc", ROCScore())
                    # setattr(pl_module, f"test_{k}_f1", F1Score())
                    # setattr(pl_module, f"test_{k}_evall", ALLScore())
                    setattr(pl_module, f"test_{k}_loss", Scalar())
...
def epoch_wrapup(pl_module, test=False):
    ...
    for loss_name, v in pl_module.hparams.config["loss_names"].items():
        ...
        if loss_name == "vqa":
            ...
            pl_module.log(f"{loss_name}/{phase}/evallscore_epoch", getattr(pl_module, f"{phase}_{loss_name}_evallscore").compute())
            getattr(pl_module, f"{phase}_{loss_name}_evallscore").reset()
            # pl_module.log(f"{loss_name}/{phase}/_roc_epoch", getattr(pl_module, f"{phase}_{loss_name}_roc").compute())
            # getattr(pl_module, f"{phase}_{loss_name}_roc").reset()
            # pl_module.log(f"{loss_name}/{phase}/_f1_epoch", getattr(pl_module, f"{phase}_{loss_name}_f1").compute())
            # getattr(pl_module, f"{phase}_{loss_name}_f1").reset()
            # pl_module.log(f"{loss_name}/{phase}/evall_epoch", getattr(pl_module, f"{phase}_{loss_name}_evall").compute())
            # getattr(pl_module, f"{phase}_{loss_name}_evall").reset()
        ...
        elif loss_name == "feaclsl":
            value = -getattr(pl_module, f"{phase}_{loss_name}_loss").compute()
            pl_module.log(f"{loss_name}/{phase}/accuracy_epoch", value)
            pl_module.log(f"{loss_name}/{phase}/loss_epoch", - value)
            getattr(pl_module, f"{phase}_{loss_name}_loss").reset()

        elif loss_name == "feaclsi":
            value = -getattr(pl_module, f"{phase}_{loss_name}_loss").compute()
            pl_module.log(f"{loss_name}/{phase}/accuracy_epoch", value)
            pl_module.log(f"{loss_name}/{phase}/loss_epoch", - value)
            getattr(pl_module, f"{phase}_{loss_name}_loss").reset()
