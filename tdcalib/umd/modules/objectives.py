...
def compute_cls(pl_module, batch, infer_unmask, test=False):
    ...
    # getattr(pl_module, f"{phase}_align_img_aucroc").update(F.sigmoid(ret["img_align_logits"]), ret["img_labels"]) for aucroc

    # roc = getattr(pl_module, f"{phase}_cls_roc")(ret["cls_logits"], ret["cls_labels"])
    # f1 = getattr(pl_module, f"{phase}_cls_f1")(ret["cls_logits"], ret["cls_labels"])
    # evall = getattr(pl_module, f"{phase}_cls_evall")(ret["cls_logits"], ret["cls_labels"])

    pl_module.log(f"cls/{phase}/loss", loss)
    pl_module.log(f"cls/{phase}/accuracy", acc)
    pl_module.log(f"cls/{phase}/evallscore", evallscore)
    # pl_module.log(f"cls/{phase}/roc", roc)
    # pl_module.log(f"cls/{phase}/f1", f1)
    # pl_module.log(f"cls/{phase}/evall", evall)

    # dict(acc=self.acc, f1=self.f1, sens=self.sens, spec=self.spec, prec=self.prec)
    return ret
