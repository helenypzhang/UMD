...
def _loss_names(d):
    ret = {
        ...
        "cls": 0,
        "irtr": 0,
        "feaclsl": 0, # four customerize losses
        "feaclsi": 0,
        "feal": 0, 
        "feai": 0,
        "bb": 0,
...
def config():
    ...
    optim_type = "adamw"
    learning_rate = 1e-5
    weight_decay = 0.01 #umd original
    # weight_decay = 0.1  #only for the first vqa dataset 
    # decay_power = 1 #umd original
    decay_power = "cosine"
    max_epoch = 100
    max_steps = 100000
    warmup_steps = 10000
...
@ex.named_config
def task_pretrain_umd():
    exp_name = "task_pretrain_umd"
    datasets = ["medicat", "roco"]
    # loss_names = _loss_names({"itm": 1, "mlm": 1, "mim": 1, "itc": 1, "bb": 1, "feaclsl": 1, "feaclsi": 1})
    loss_names = _loss_names({"itm": 1, "mlm": 1, "mim": 1, "itc": 1, "bb": 1})
    batch_size = 256
    max_epoch = 10
    max_steps = 100000
    warmup_steps = 0.1
...
def task_finetune_vqa_vqa_rad():
    ...
    loss_names = _loss_names({"mim": 1, "mlm": 1, "feai": 1, "feal": 1})
    # loss_names = _loss_names({"mim": 1, "mlm": 1})
    # loss_names = _loss_names({"vqa": 1})
    # batch_size = 64 # original umd
    batch_size = 32
    # max_epoch = 50 * 2
    max_epoch = 60
    max_steps = None
    warmup_steps = 0.1
    draw_false_image = 0
    learning_rate = 5e-6 # original umd
...
def task_finetune_irtr_roco():
    ...
    # batch_size = 256 #original umd, 128is good
    batch_size = 128
    max_epoch = 20 
    max_steps = None
    warmup_steps = 0.1
